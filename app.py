...
def analyze():
    ...
    try:
        ...
        # 4. DETEKSI (ALERGI & HALAL)
        # SAYA SAMAKAN NAMANYA: 'allergens'
        allergens = detect_allergen(normalized) 
        status_halal, critical_list, haram_list = detect_halal_status(normalized)

        # 5. LOGIKA PERSONALISASI (COCOKKAN DENGAN USER)
        # Gunakan variabel 'allergens' yang sudah didefinisikan di atas
        ...
        if is_low_quality:
            ...
            if is_critical:
                final_message += f" Selain itu, ada bahan titik kritis: {', '.join(critical_list)}."
        ...
        return jsonify({
            ...
            "allergens": allergens, # Menggunakan nama yang konsisten
